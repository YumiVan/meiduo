    ...
    def post(self, request):
        ...
        if user.is_authenticated:
            ...
            if selected:  # 只有勾选的才向set集合中添加
                ...
                # pl.expire('selected_%s' % user.id, 3600)
            ...
            # 如果是未登陆用户存储购物车数据到cookie
            '''
            {
                sku_id_1:{'count':2,'selected':Ture}
                sku_id_2:{'count':2,'selected':Ture}
            }
            '''
            # 先获取cookie中购物车数据
            ...
            # 判断要添加的sku_id在字典key中是否存在,如果存在,需要对count做增量计算
